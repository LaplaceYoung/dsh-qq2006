 ...
 */
export const QQ2006_TOKENS = Object.freeze({
...
export function apply(ctx) {
    ...
    if (typeof localStorage !== 'undefined') {
        try {
            if (localStorage.getItem('dsh.theme') === THEME_ID)
    ...
    const sync = (snapshot) => {
        ...
        if (typeof document === 'undefined')
            return;
        if (snapshot.active.id === THEME_ID)
            document.body.setAttribute(SKIN_ATTRIBUTE, THEME_ID);
        else
