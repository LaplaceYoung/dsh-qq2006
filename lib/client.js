window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		const QQ2006_TOKENS = Object.freeze({
		...
		*/
		function apply(ctx) {
			...
			if (typeof localStorage !== "undefined") try {
				if (localStorage.getItem("dsh.theme") === "qq2006") ctx.theme.setTheme(THEME_ID);
			} catch {}
			const sync = (snapshot) => {
				if (typeof document === "undefined") return;
				if (snapshot.active.id === "qq2006") document.body.setAttribute(SKIN_ATTRIBUTE, THEME_ID);
		...
		//#endregion
		exports.QQ2006_TOKENS = QQ2006_TOKENS;
